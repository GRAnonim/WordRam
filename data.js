/**
 * WordRam - Data & Pedagogical Architecture (v28)
 * 1 500 слов CEFR, 10 семантических тем, 100% аутентичный подбор слов,
 * рамка-лоток игрового поля, Слово дня и чистая терминология.
 */
...
const WordRamData = {
  ...
  getLevelPackingConfig(levelNumber, userCefr = "A2") {
    ...
    // Выбираем семантическую тему уровня
  ...
  getWordForCefrAndLength(cefrLevel, targetLen, exclude = [], themeKey = null) {
    ...
    // 1. Приоритет 1: слова заявленной ТЕМЫ на текущем уровне игрока
    ...
    // 2. Приоритет 2: слова заявленной ТЕМЫ из более легких уровней
    if (themeKey && this.themes[themeKey]) {
      ...
      for (let i = userRankIdx - 1; i >= 0; i--) {
        ...
        if (this.cefrDictionary[lowerLvl] && this.cefrDictionary[lowerLvl][targetLen]) {
          const themedAvailable = this.cefrDictionary[lowerLvl][targetLen].filter(
            w => themeWords.includes(w) && !exclude.includes(w) && w.length === targetLen
          );
          if (themedAvailable.length > 0) {
            return themedAvailable[Math.floor(Math.random() * themedAvailable.length)];
    ...
    // 3. Приоритет 3: любое слово на текущем уровне игрока
    ...
    // 4. Закрепление: любое слово из уровней ниже
    for (let i = userRankIdx - 1; i >= 0; i--) {
      const lowerLvl = rankOrder[i];
      if (this.cefrDictionary[lowerLvl] && this.cefrDictionary[lowerLvl][targetLen]) {
    ...
    // 5. Для гарантированной уникальности
    for (let i = userRankIdx + 1; i < rankOrder.length; i++) {
      const higherLvl = rankOrder[i];
      if (this.cefrDictionary[higherLvl] && this.cefrDictionary[higherLvl][targetLen]) {
        const available = this.cefrDictionary[higherLvl][targetLen].filter(
          w => !exclude.includes(w) && w.length === targetLen
        );
        if (available.length > 0) {
          return available[Math.floor(Math.random() * available.length)];
    ...
    // 6. Любое слово нужной длины
    for (const lvl of rankOrder) {
      if (this.cefrDictionary[lvl] && this.cefrDictionary[lvl][targetLen] && this.cefrDictionary[lvl][targetLen].length > 0) {
        const list = this.cefrDictionary[lvl][targetLen];
